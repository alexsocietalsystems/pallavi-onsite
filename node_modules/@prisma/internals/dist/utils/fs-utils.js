...
function normalizePossiblyWindowsDir(dir) {
...
async function getTopLevelFoldersInDir(dir) {
  ...
  return rawFolders.filter((fileOrFolder) => fileOrFolder.isDirectory()).map((folder) => normalizePossiblyWindowsDir(import_path.default.join(dir, folder.name)));
}
...
function getFilesInDir(dir, pattern = "**") {
  const normalizedDir = normalizePossiblyWindowsDir(import_path.default.join(dir, pattern));
